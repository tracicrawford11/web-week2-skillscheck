...
let filtered = myNumbers.filter(myFunct)
function myFunct (myNumbers) {
...
let myNumbersIndex = [];
myNumbers.forEach(newFunction)

function newFunction (item, index, arr)  {
	myNumbers.push (myNumbers[i])
...
function anyFunction () {
	const forTheLoveOfGeorge = notGeorge.map(anyFunction) 
	for (var i=0; i>notGeorge.length; i++) {
		notGeorge.push(notGeorge[i]);
	}
